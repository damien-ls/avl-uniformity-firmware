...
void DRV_USART3_TasksTransmit(void)
{
    ...
    if(SYS_INT_SourceStatusGet(INT_SOURCE_USART_4_TRANSMIT))
    {
        /* The USART driver is configured to generate an
           ...
           the queue is not empty. Which means there is
           work to be done in this routine. */
        _DRV_USART3_BufferQueueTxTasks();
...
void DRV_USART3_TasksReceive(void)
{
    ...
    {
        _DRV_USART3_BufferQueueRxTasks();
...
void DRV_USART3_TasksError(void)
{
    ...
    {
        /* This means an error has occurred */
        _DRV_USART3_BufferQueueErrorTasks();
        /* Clear up the error interrupt flag */
...
void _DRV_USART3_BufferQueueRxTasks(void)
{
    DRV_USART_BUFFER_OBJ * bufferObj;
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
...
void _DRV_USART3_BufferQueueTxTasks(void)
{
    /* Start by getting the buffer at the head of queue. */
...
void _DRV_USART3_BufferQueueErrorTasks(void)
{
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
    DRV_USART_BUFFER_OBJ * bufferObj;
    bool mutexGrabbed  = true;
    ...
    if(true == mutexGrabbed)
    {
        if(bufferObj != NULL)
        {
	        ...
	        /* Clear error condition */
            _DRV_USART3_ErrorConditionClear();
...
void DRV_USART4_TasksTransmit(void)
{
    ...
    if(SYS_INT_SourceStatusGet(INT_SOURCE_USART_5_TRANSMIT))
    {
        /* The USART driver is configured to generate an
           ...
           the queue is not empty. Which means there is
           work to be done in this routine. */
        _DRV_USART4_BufferQueueTxTasks();
...
void DRV_USART4_TasksReceive(void)
{
    ...
    {
        _DRV_USART4_BufferQueueRxTasks();
...
void DRV_USART4_TasksError(void)
{
    ...
    {
        /* This means an error has occurred */
        _DRV_USART4_BufferQueueErrorTasks();
        /* Clear up the error interrupt flag */
...
void _DRV_USART4_BufferQueueRxTasks(void)
{
    DRV_USART_BUFFER_OBJ * bufferObj;
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
    ...
    if(bufferObj != NULL)
    {
        ...
        while((PLIB_USART_ReceiverDataIsAvailable(USART_ID_5))
                && (bufferObj->nCurrentBytes < bufferObj->size ))
        {
            bufferObj->buffer[bufferObj->nCurrentBytes] = PLIB_USART_ReceiverByteReceive(USART_ID_5);
            bufferObj->nCurrentBytes ++;
        }
...
void _DRV_USART4_BufferQueueTxTasks(void)
{
    /* Start by getting the buffer at the head of queue. */
    ...
    if(dObj->queueWrite != NULL)
    {
        ...
        while((!PLIB_USART_TransmitterBufferIsFull(USART_ID_5))
                && (bufferObj->nCurrentBytes < bufferObj->size ))
        {
            PLIB_USART_TransmitterByteSend(USART_ID_5, bufferObj->buffer[bufferObj->nCurrentBytes]);
            bufferObj->nCurrentBytes ++;
        }
    }
...
void _DRV_USART4_BufferQueueErrorTasks(void)
{
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
    DRV_USART_BUFFER_OBJ * bufferObj;
    bool mutexGrabbed  = true;
    ...
    if(true == mutexGrabbed)
    {
        ...
        {
            /* Get the USART errors */
            dObj->error = PLIB_USART_ErrorsGet(USART_ID_5);

	        /* Clear error condition */
            _DRV_USART4_ErrorConditionClear();
...
SYS_MODULE_OBJ DRV_USART5_Initialize(void)
{
    ...
    /* Return the driver instance value*/
    return (SYS_MODULE_OBJ)DRV_USART_INDEX_5;
}
...
void DRV_USART5_TasksTransmit(void)
{
    ...
    if(SYS_INT_SourceStatusGet(INT_SOURCE_USART_6_TRANSMIT))
    {
        /* The USART driver is configured to generate an
           ...
           the queue is not empty. Which means there is
           work to be done in this routine. */
        _DRV_USART5_BufferQueueTxTasks();
...
void DRV_USART5_TasksReceive(void)
{
    ...
    {
        _DRV_USART5_BufferQueueRxTasks();
...
void DRV_USART5_TasksError(void)
{
    ...
    {
        /* This means an error has occurred */
        _DRV_USART5_BufferQueueErrorTasks();
        /* Clear up the error interrupt flag */
...
DRV_HANDLE DRV_USART5_Open(const SYS_MODULE_INDEX index, const DRV_IO_INTENT ioIntent)
{
    ...
    /* Return the driver instance value*/
    return ((DRV_HANDLE)DRV_USART_INDEX_5 );
}
...
{
    DRV_USART_ERROR error;
    error = gDrvUSART5Obj.error;

    /* Clear the error before returning */
    gDrvUSART5Obj.error = DRV_USART_ERROR_NONE;
...
void _DRV_USART5_BufferQueueRxTasks(void)
{
    DRV_USART_BUFFER_OBJ * bufferObj;
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
    ...
    if(bufferObj != NULL)
    {
        ...
        while((PLIB_USART_ReceiverDataIsAvailable(USART_ID_6))
                && (bufferObj->nCurrentBytes < bufferObj->size ))
        {
            bufferObj->buffer[bufferObj->nCurrentBytes] = PLIB_USART_ReceiverByteReceive(USART_ID_6);
            bufferObj->nCurrentBytes ++;
        }
...
void _DRV_USART5_BufferQueueTxTasks(void)
{
    /* Start by getting the buffer at the head of queue. */
    ...
    if(dObj->queueWrite != NULL)
    {
        ...
        while((!PLIB_USART_TransmitterBufferIsFull(USART_ID_6))
                && (bufferObj->nCurrentBytes < bufferObj->size ))
        {
            PLIB_USART_TransmitterByteSend(USART_ID_6, bufferObj->buffer[bufferObj->nCurrentBytes]);
            bufferObj->nCurrentBytes ++;
        }
    }
...
void _DRV_USART5_BufferQueueErrorTasks(void)
{
    DRV_USART_OBJ *dObj = (DRV_USART_OBJ*)NULL;
    DRV_USART_BUFFER_OBJ * bufferObj;
    bool mutexGrabbed  = true;
    ...
    if(true == mutexGrabbed)
    {
        ...
        {
            /* Get the USART errors */
            dObj->error = PLIB_USART_ErrorsGet(USART_ID_6);

	        /* Clear error condition */
            _DRV_USART5_ErrorConditionClear();
...
void _DRV_USART5_ErrorConditionClear()
{
    uint8_t dummyData = 0u;
    /* RX length = (FIFO level + RX register) */
    uint8_t RXlength = _DRV_USART_RX_DEPTH;
